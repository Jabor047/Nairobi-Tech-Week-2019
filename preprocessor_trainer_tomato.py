...
def preprocessing(dim):
	folders = glob.glob("PlantVillage-Dataset/raw/color/*")
	parts = [Path(folder).parts[3] for folder in folders]
	
	needed = ['Tomato'] #'Potato', 'Pepper','Corn'
...
def train(X, Y, classes, whichOne):
	...
	if(whichOne == 'alexnet'):
		if(os.path.exists('agrix_alexnet_tomato.h5')):
		...
		if(os.path.exists('some_agrix_nn_tomato.h5')):
	...
	model.fit_generator(aug.flow(XTrain, yTrain, batch_size = 32), epochs = 15, validation_data = (XTest, yTest), steps_per_epoch = len(XTrain) // 32, verbose = 1)
	...
	if(whichOne == 'alexnet'):			
		model.save('agrix_alexnet_tomato.h5')
		logger.info('Saved alexnet to file.')
	else:
		model.save('some_agrix_nn_tomato.h5')
