...
def preprocessing(dim):
	...
	needed = ['Pepper'] #'Tomato', 'Potato','Corn'
...
def train(X, Y, classes, whichOne):
	...
	if(whichOne == 'alexnet'):
		if(os.path.exists('agrix_alexnet_pepper.h5')):
		...
		if(os.path.exists('some_agrix_nn_pepper.h5')):
	...
	if(whichOne == 'alexnet'):			
		model.save('agrix_alexnet_pepper.h5')
		logger.info('Saved alexnet to file.')
	else:
		model.save('some_agrix_nn_pepper.h5')
