...
def preprocessing(dim):
	...
	needed = ['Corn'] #'Tomato', 'Potato','Pepper'
...
def train(X, Y, classes, whichOne):
	...
	if(whichOne == 'alexnet'):
		if(os.path.exists('agrix_alexnet_corn.h5')):
		...
		if(os.path.exists('some_agrix_nn_corn.h5')):
	...
	if(whichOne == 'alexnet'):			
		model.save('agrix_alexnet_corn.h5')
		logger.info('Saved alexnet to file.')
	else:
		model.save('some_agrix_nn_corn.h5')
